...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if is_deepspeed_zero3_enabled():
            ...
            # If PEFT configuration is not provided, create a reference model based on
            # the initial model.
        ...
        for i, (reward_processing_class, reward_func) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                # So it's important to set the pad token ID to the padding token ID of the
                # processing class.
        ...
        if self.accelerator.is_main_process:
            ...
            vllm_device = "auto"
            if vllm_device == "auto":
                # vllm_device = f"cuda:{self.accelerator.num_processes - 1}"  # take the
                # next GPU idx
                vllm_device = "cuda:7"
            ...
            with world_size_patch, profiling_patch:
                self.llm = LLM(
                    ...
                    # This is particularly useful here because we generate completions from
                    # the same prompts.
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.max_prompt_length is not None:
            ...
            batched_inputs["pixel_values"] = batched_inputs["pixel_values"][:, -
        ...
        for i, reward_func in enumerate(self.reward_funcs):
            # Repeat all input columns (but "prompt" and "completion") to match the
            # number of generations
            reward_kwargs = {key: []
                             for key in inputs[0].keys() if key not in ["prompt", "completion"]}
            ...
            if i < 2:
                ...
                # The last reward function should be the length of the completions
                output_reward_func = reward_func(
                    completions=completions, former_reward=rewards_per_func[:, :i], **reward_kwargs)
                rewards_per_func[:, i] = torch.tensor(
                    output_reward_func, dtype=torch.float32, device=device)
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        metrics = {key: sum(val) / len(val)
                   for key, val in self._metrics.items()}  # average the metrics
        logs = {**logs, **metrics}
